...
class ArtefactDetails extends React.Component {
	...
	render() {
		...
		case Constants.DATA_REQUEST_STATUS.LOADING:
			return_value = (
				<div className={Constants.DISPLAY_ARTEFACT_MESSAGE}>
					<p>Loading...</p>
				</div>
			);
			break;
		...
		case Constants.DATA_REQUEST_STATUS.FAILURE:
			return_value = (
				<div className={Constants.DISPLAY_ARTEFACT_MESSAGE_ERROR}>
		...
		case Constants.DATA_REQUEST_STATUS.SUCCESS:
			return_value = (
				<div
					className={Constants.DISPLAY_ARTEFACT_CONTENT}
					id={Constants.DISPLAY_ARTEFACT_CONTENT}
				>
					<ArtefactName objectName={this.props.objectData.objectName} />
					<ArtefactSlug objectSlug={this.props.objectData.objectSlug} />
					<ArtefactPrimaryImage
						objectPrimaryImageURL={
							this.props.objectData.objectPrimaryImageURL
						}
					/>
					<ImageGallery
						imageUrls={this.props.objectData.objectImages}
						onClick={this.props.onClick}
					/>
					<ArtefactText objectText={this.props.objectData.objectText} />
					<ArtefactFullText
						objectFullText={this.props.objectData.objectFullText}
						objectFullDetails={this.props.objectData.objectFullDetails}
					/>
				</div>
			);
			break;
...
class ArtefactName extends React.Component {
	render() {
		return <h2>{this.props.objectName}</h2>;
	}
}

class ArtefactSlug extends React.Component {
	render() {
		return <p>{this.props.objectSlug}</p>;
	}
}
...
class ArtefactPrimaryImage extends React.Component {
	componentDidUpdate(prevProps) {
		...
		if (
			element &&
			this.props.objectPrimaryImageURL !== prevProps.objectPrimaryImageURL
		) {
			//alert("test");
			element.scrollIntoView({
				block: 'start',
	...
	render() {
		if (this.props.objectPrimaryImageURL) {
			return (
				<div className={Constants.DISPLAY_PRIMARY_IMAGE_WRAPPER}>
					<img
						className={Constants.DISPLAY_PRIMARY_IMAGE}
						src={this.props.objectPrimaryImageURL}
...
class ArtefactText extends React.Component {
	render() {
		return <p>{this.props.objectText}</p>;
	}
}
...
function DisplayDetails(detail, index) {
	...
	if (Array.isArray(detail.text)) {
		...
		if (detail.text.length === 1) {
			subgroup = artefactDetail.text[0].map((detail, index) =>
				DisplayDetails(detail, index)
			);
		} else {
			subgroup = artefactDetail.text.map((detail, index) =>
				DisplayDetailsSubGroup(detail, index)
			);
		}

		return (
			<div key={index} className={Constants.DISPLAY_ARTEFACT_DETAILS_GROUP}>
				<h3 className={Constants.DISPLAY_ARTEFACT_DETAILS_SUBHEADING}>
					{artefactDetail.title}:
				</h3>
				<div className={Constants.DISPLAY_ARTEFACT_DETAILS_SUBGROUP}>
					{subgroup}
				</div>
			</div>
		);
		...
		return (
			<div key={index} className={Constants.DISPLAY_ARTEFACT_DETAILS_GROUP}>
				<h3 className={Constants.DISPLAY_ARTEFACT_DETAILS_HEADING}>
					{artefactDetail.title}:
				</h3>
				<p className={Constants.DISPLAY_ARTEFACT_DETAILS_TEXT}>
					{artefactDetail.text}
				</p>
			</div>
		);
...
function DisplayDetailsSubGroup(detail, index) {
	const subgroupdetails = detail.map((inner_detail, index) =>
		DisplayDetails(inner_detail, index)
	);
