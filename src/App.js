...
class App extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      searchResultsEndRange: parseInt(Constants.DEFAULT_NO_OF_RESULTS_TO_SHOW, 10),
      ...
      searchResultsBackgroundDataRequestStatus: Constants.DATA_REQUEST_STATUS.NONE_MADE,
    }
  ...
  requestData(searchType, selectedSource, searchText) {
    getData(searchType, selectedSource, searchText, this.state.searchResultsProcessedData.length)
      .then(returnData => {
        ...
        if (returnData[Constants.DATA_REQUEST_NUMBER_RESULTS]) {
          var searchResultsRawExtended = returnData[Constants.DATA_REQUEST_RAW_DATA];
          //if (this.state[rawData] !== null) {
          //  searchResultsRawExtended = this.state[rawData].concat(returnData[Constants.DATA_REQUEST_RAW_DATA]);
          //}

          var searchResultsProcessedExtended = returnData[Constants.DATA_REQUEST_PROCESSED_DATA];
          if (this.state[processedData]) {
            searchResultsProcessedExtended = this.state[processedData].concat(returnData[Constants.DATA_REQUEST_PROCESSED_DATA]);
          }

          var maxNumberToDisplay = returnData[Constants.DATA_REQUEST_NUMBER_RESULTS] < this.state.searchResultsEndRange ? returnData[Constants.DATA_REQUEST_NUMBER_RESULTS] : this.state.searchResultsEndRange;
          var newSearchResultRequestLimit = this.state.searchResultsEndRange + Constants.DATA_RESULTS_DOWNLOAD_LIMIT <= returnData[Constants.DATA_REQUEST_NUMBER_RESULTS] ? this.state.searchResultsEndRange + Constants.DATA_RESULTS_DOWNLOAD_LIMIT : returnData[Constants.DATA_REQUEST_NUMBER_RESULTS];

          console.log("1st branch - " + searchResultsProcessedExtended.length + ", " + returnData[Constants.DATA_REQUEST_NUMBER_RESULTS]);
          this.setState({
            ...
            searchResultsTotalNumber: returnData[Constants.DATA_REQUEST_NUMBER_RESULTS],
            ...
            searchResultsBackgroundDataRequestStatus: returnData[Constants.DATA_REQUEST_RESULT],
          });
        } else {
          console.log("2nd branch - " + this.state.searchResultsTotalNumber);
          this.setState({
            [dataRequestStatus]: returnData[Constants.DATA_REQUEST_RESULT],
            [errorMessage]: returnData[Constants.DATA_REQUEST_ERROR],
      ...
      .then(() => {
        if (this.state.searchResultsProcessedData && this.state.searchResultsProcessedData.length < this.state.searchResultRequestLimit && this.state.searchResultsProcessedData.length < this.state.searchResultsTotalNumber) { 
          this.setState({
            searchResultsBackgroundDataRequestStatus: Constants.DATA_REQUEST_STATUS.LOADING,
          });
  ...
  handleClick(event) {
    ...
    switch (category) {
      ...
      case Constants.PREVIOUS_BUTTON:
        newStartRange = this.state.searchResultsStartRange - parseInt(this.state.selectedNumSearchResults, 10);
        if (newStartRange < 1) {newStartRange = 1}
        newEndRange = newStartRange + parseInt(this.state.selectedNumSearchResults, 10) - 1;
        if (newEndRange > this.state.searchResultsTotalNumber) {newEndRange = this.state.searchResultsTotalNumber}
      ...
      case Constants.NEXT_BUTTON:
        newStartRange = this.state.searchResultsStartRange + parseInt(this.state.selectedNumSearchResults, 10);
        if (newStartRange > this.state.searchResultsTotalNumber) {newStartRange = this.state.searchResultsTotalNumber}
        newEndRange = newStartRange + parseInt(this.state.selectedNumSearchResults, 10) - 1;
        if (newEndRange > this.state.searchResultsTotalNumber) {newEndRange = this.state.searchResultsTotalNumber}
        //console.log(newEndRange);
        var newSearchResultRequestLimit = newEndRange + Constants.DATA_RESULTS_DOWNLOAD_LIMIT <= this.state.searchResultsTotalNumber ? newEndRange + Constants.DATA_RESULTS_DOWNLOAD_LIMIT : this.state.searchResultsTotalNumber;
        ...
        if (this.state.searchResultsBackgroundDataRequestStatus !== Constants.DATA_REQUEST_STATUS.LOADING && this.state.searchResultsProcessedData.length <= newSearchResultRequestLimit && this.state.searchResultsProcessedData.length < this.state.searchResultsTotalNumber) {
          this.requestData(this.state.lastSearchType, this.state.lastSelectedSource, this.state.lastSearchText);
        }
      break;
  ...
  handleChange(event) {
    switch (event.target.id) {
      ...
      case Constants.SEARCH_RESULTS_NUMBER_SELECT:
        var newSearchResultRequestLimit = this.state.searchResultsStartRange + parseInt(event.target.value, 10) - 1 + Constants.DATA_RESULTS_DOWNLOAD_LIMIT <= this.state.searchResultsTotalNumber ? this.state.searchResultsStartRange + parseInt(event.target.value, 10) - 1 + Constants.DATA_RESULTS_DOWNLOAD_LIMIT : this.state.searchResultsTotalNumber;
        if (event.target[event.target.selectedIndex].text === Constants.NO_OF_RESULTS_TO_SHOW.ALL) {
          this.setState({
          ...
          this.setState({
            selectedNumSearchResults: event.target.value,
            searchResultsEndRange: this.state.searchResultsStartRange + parseInt(event.target.value, 10) - 1,
            searchResultRequestLimit: newSearchResultRequestLimit,
          });
        }
        if (this.state.searchResultsBackgroundDataRequestStatus !== Constants.DATA_REQUEST_STATUS.LOADING && this.state.searchResultsProcessedData.length <= newSearchResultRequestLimit && this.state.searchResultsProcessedData.length < this.state.searchResultsTotalNumber) {
    ...
    return (
      <div className = "App">
        ...
        <div className="external-data">
          <SearchResults dataRequestStatus={this.state.searchResultsDataRequestStatus} searchData={this.state.searchResultsProcessedData} errorMessage={this.state.searchResultsErrorMessage} selectedNumSearchResults={this.state.selectedNumSearchResults} searchResultsStartRange={this.state.searchResultsStartRange} searchResultsEndRange={this.state.searchResultsEndRange} searchResultsTotalNumber={this.state.searchResultsTotalNumber} onChange={this.handleChange} onClick={this.handleClick} />
